...
from lib.gui.widgets import Grid, CloseButton, Label, Button
from lib.gui.core.writer import CWriter

# Font for CWriter
import lib.gui.fonts.font10 as font
import lib.gui.fonts.font14 as font1
...
class BaseScreen(Screen):
    ...
    def __init__(self):
        ...
        colwidth = 35
        self.lbl = Label(
            wri, row, col, text=(colwidth + 4) * cols, justify=Label.CENTRE
        )
        row = self.lbl.mrow
    ...
    def update(self):
        ...
        today = DateCal()
        if (
            cur.year == today.year and cur.month == today.month
        ):  # Today is in current month
            grid[idx_1 + today.mday - 1] = {"fgcolor": RED}
        self.lblnow.value(
            f"{today.day_str} {today.mday} {today.month_str} {today.year}"
        )
