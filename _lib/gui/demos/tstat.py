...
# Usage:
# import lib.guidemos.tstat

# hardware_setup must be imported before other modules because of RAM use.
import hardware_setup  # Create a display instance
from lib.gui.core.ugui import Screen, ssd

from lib.gui.widgets import Button, CloseButton, Slider, Label, Meter, Region, LED
...
class BaseScreen(Screen):
    ...
    def __init__(self):
        ...
        self.led = LED(
            wri, self.alm.mrow + 5, col, height=20, color=YELLOW, bdcolor=BLACK
        )
        self.grn = LED(
            wri, self.led.mrow + 5, col, height=20, color=GREEN, bdcolor=BLACK
        )
        col = self.lbl.mcol + 5
        btn = Button(
            wri,
    ...
    def al_cb(self, reg, reason):
        if reason == reg.EN_WB or reason == reg.T_IA:  # Logical OR
            ...
            reg.EX_WB_IB | reg.EX_WA_IB | reg.T_IB
        ):  # Bitwise OR alternative
            self.alm(False)
